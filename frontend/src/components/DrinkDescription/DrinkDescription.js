...
export const DrinkDescription = ({drink}) => {
  const date = new Date(drink?.date_modified);
  const dateAndTime = `${date.toLocaleDateString()} ${date.toLocaleTimeString()}`;
  ...
  return (
    <Descriptions title="Drink Info" layout="vertical" bordered>
      ...
      <Descriptions.Item label="Date Modified">{dateAndTime}</Descriptions.Item>
      {
        drink?.tags?.length ?
